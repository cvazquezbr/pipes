...
export function extractTaxMappings(data: ExcelReferenceData[]): TaxMapping[] {
  return data.map(row => {
    ...
    if (itemTax1Percent && !isNaN(Number(itemTax1Percent.replace(",", ".")))) {
      itemTax1Percent = parseFloat(itemTax1Percent.replace(",", ".")).toFixed(
        2
      );
    }
 ...
 */
export function extractClientMappings(
  data: ExcelReferenceData[]
): ClientMapping[] {
  return data.map(row => {
    const keys = Object.keys(row);
    return {
      de: String(row["DE"] || row[keys[0]] || "")
        .trim()
        .toUpperCase(),
 ...
 */
export function extractAllocationData(
  data: ExcelReferenceData[]
): AllocationData[] {
  return data.map(row => {
    const keys = Object.keys(row);
    ...
    const rawDays =
      row["Vencimento"] !== undefined && row["Vencimento"] !== ""
        ? row["Vencimento"]
    ...
    return {
      ...
      projeto: String(
        row["Projeto"] || row["Project Name"] || row[keys[2]] || ""
      ),
      dueDateDays,
    };
  });
...
function findTaxMapping(
  ...
  console.log(
    `[ZOHO] Buscando esquema de tributação para ${retentionPercentage.toFixed(2)}%...`
  );
...
function findClientMapping(
  ...
  console.log("[ZOHO] Buscando cliente:", {
    originalName,
    normalized,
    totalMappings: clientMappings.length,
  });
  if (clientMappings.length > 0) {
    console.log(
      "[ZOHO] Primeiros 3 mapeamentos:",
      clientMappings.slice(0, 3).map(m => ({ de: m.de, para: m.para }))
    );
  }
  const result = clientMappings.find(m => m.de === normalized);
  console.log(
    "[ZOHO] Resultado do match:",
    result ? result.para : "NAO ENCONTRADO"
  );
  return result || null;
}
...
export function convertToZOHO(
  ...
  const specialAccount = determineAccountFromDescription(
    invoice.serviceDescription
  );
  if (specialAccount) {
    console.log(
      `[ZOHO] Conta ajustada por descrição: ${account} -> ${specialAccount}`
    );
...
export function exportToZOHOExcel(
  ...
  if (allSheets) {
    ...
    if (sheetNames.length > 2 && allSheets[sheetNames[2]]) {
      console.log(
        `[ZOHO] Extraindo dados de alocação da 3ª aba: ${sheetNames[2]}`
      );
  ...
  filteredInvoices.forEach(invoice => {
    ...
    if (!invoice.takerName) {
      issues.push({
        ...
        issue: "Nome do tomador não extraído",
      });
    }
    if (invoice.serviceValue === 0) {
      issues.push({
        nfsNumber: invoice.nfsNumber,
        issue: "Valor do serviço é zero",
      });
    }
